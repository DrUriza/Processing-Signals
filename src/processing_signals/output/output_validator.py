...
class OutputValidator:
    ...
    def _validate_family_file(
        ...
        block_reports = [self._validate_block(json_path, block, family_key, output_shape, errors, warnings) for block in blocks]
    ...
    def _validate_block(
        ...
        missing_keys = sorted(self.REQUIRED_BLOCK_KEYS - set(block.keys()))
        for key in missing_keys:
            errors.append(f"{json_path}: block {block.get('source_name')} missing key {key}")

        if block.get("family_key") != family_key and family_key != "volume_orderflow":
